...
1.1. **快照分级（③ 自决策，05 分册）**：按操作破坏性定快照时机——**必拍**：合并/
   栅格化/删层/模式转换/含降采样的裁剪；**应拍**：滤镜、大面积像素调整；**免拍**：
   纯调整层参数与视图操作。分级判断进 decisions，置信度不足按"必拍"处理；
...
3. **修正**：异常→调参或换法重跑**该操作**（副本上，快照纪律不变）；
   **修正指令必须引用所见**——不许写"重跑一下"，必须写"v1485 段 -4px 处有 α≈0.04 的暗带，故二次
   ...
   带着瓦片升 G1 交用户裁决；
4. **失败路由三分类**（05 分册 §4）：修正前先分类——**参数问题**（方向对、程度不对）
   →调参重跑；**方法问题**（方向就不对）→换法重跑并回填工艺库；**理解问题**（目标
   存疑）→停下问人；判不准归哪类＝按理解问题处理；
5. **留痕**：瓦片文件名+判定结论+失败分类进 report 实看清单与 decisions 节
   （存在性与文件系统互证）。
